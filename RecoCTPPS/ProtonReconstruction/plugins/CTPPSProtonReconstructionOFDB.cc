...
class CTPPSProtonReconstructionOFDB : public edm::stream::EDProducer<>
{
  public:
    explicit CTPPSProtonReconstructionOFDB(const edm::ParameterSet&);
...
CTPPSProtonReconstructionOFDB::CTPPSProtonReconstructionOFDB( const edm::ParameterSet& iConfig ) :
  ...
  if (applyExperimentalAlignment)
  {
    for (const auto &f : alignmentFiles_)
    {
      ...
      if (alignmentCollection_.Load(fip.fullPath()) != 0)
        throw cms::Exception("CTPPSProtonReconstructionOFDB") << "Cannot load alignment from file '" << f << "'.";
    }
  }
...
void CTPPSProtonReconstructionOFDB::fillDescriptions(ConfigurationDescriptions& descriptions)
{
...
CTPPSProtonReconstructionOFDB::~CTPPSProtonReconstructionOFDB()
{
}

//----------------------------------------------------------------------------------------------------

void CTPPSProtonReconstructionOFDB::produce(Event& event, const EventSetup &eventSetup)
{
  // get conditions
...
DEFINE_FWK_MODULE(CTPPSProtonReconstructionOFDB);
