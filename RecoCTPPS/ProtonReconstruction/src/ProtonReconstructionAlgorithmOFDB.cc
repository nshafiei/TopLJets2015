...
void ProtonReconstructionAlgorithmOFDB::Init(double xangle)
{
  Release();

  printf(">> ProtonReconstructionAlgorithmOFDB::Init(%.1f)\n", xangle);
  if (xangle == 0)
  {
    printf("ERROR: no xangle set, no reconstruction will be made\n");
    return;
  }

#ifdef DEBUG
  char buf[100];
  sprintf(buf, "debug_xangle_%.1f.root", xangle);
  TFile *f_debug = new TFile(buf, "recreate");
#endif
  ...
  for (const auto &it : idNameMap)
  {
    ...
    // load optics
    LHCOpticsApproximatorOF oa1(opticsFile1, ofName.c_str());
    LHCOpticsApproximatorOF oa2(opticsFile2, ofName.c_str());

    rpod.optics = LHCOpticsApproximatorOF::Interpolate(xangle1, oa1, xangle2, oa2, xangle);
    ...
    // input: all zero
    double kin_in_zero[5] = { 0., 0., vtx0_y, 0., 0. };
    double kin_out_zero[5] = { 0., 0., 0., 0., 0. };
    rpod.optics->Transport(kin_in_zero, kin_out_zero);
    ...
    for (double xi = 0.; xi <= 0.3001; xi += 0.005)
    {
      ...
      double kin_out_xi[5];
        rpod.optics->Transport(kin_in_xi, kin_out_xi);

      // input: xi and th_x
      const double th_x = 20E-6;  // rad
      double kin_in_xi_th_x[5] = { 0., th_x, vtx0_y, 0., xi };
      double kin_out_xi_th_x[5];
        rpod.optics->Transport(kin_in_xi_th_x, kin_out_xi_th_x);

      // input: xi and vtx_y
      const double vtx_y = 10E-6;  // m
      double kin_in_xi_vtx_y[5] = { 0., 0., vtx0_y + vtx_y, 0., xi };
      double kin_out_xi_vtx_y[5];
        rpod.optics->Transport(kin_in_xi_vtx_y, kin_out_xi_vtx_y);

      // input: xi and th_y
      const double th_y = 20E-6;  // rad
      double kin_in_xi_th_y[5] = { 0., 0., vtx0_y, th_y, xi };
      double kin_out_xi_th_y[5];
        rpod.optics->Transport(kin_in_xi_th_y, kin_out_xi_th_y);
...
void ProtonReconstructionAlgorithmOFDB::Release()
{
  for (auto &p : m_rp_optics_)
  {
    delete p.second.optics;

    delete p.second.s_xi_vs_x;
    delete p.second.s_y0_vs_xi;
    delete p.second.s_v_y_vs_xi;
    delete p.second.s_L_y_vs_xi;
  }
...
double ProtonReconstructionAlgorithmOFDB::ChiSquareCalculator::operator() (const double* parameters) const
{
  ...
  for (const auto &track : *tracks)
  {
    ...
    double kin_in[5] = { vtx_x, th_x, vtx0_y + vtx_y, th_y, xi };
    double kin_out[5];
...
void ProtonReconstructionAlgorithmOFDB::reconstructFromMultiRP(const vector<const CTPPSLocalTrackLite*> &tracks, vector<reco::ProtonTrack> &out) const
{
  if (!initialized)
    return;
  ...
  for (const auto &it : tracks)
  {
    ...
    if (oit == m_rp_optics_.end())
      throw cms::Exception("") << "Optics data not available for RP " << it->getRPId() << ", i.e. " << CTPPSDetId(it->getRPId()) << ".";
  }
  ...
  if (verbosity)
  {
    ...
    printf("* ProtonReconstructionAlgorithmOFDB::reconstructFromMultiRP(%u)\n", armId);
  ...
  if (verbosity)
    edm::LogInfo("ProtonReconstructionAlgorithmOFDB")
...
void ProtonReconstructionAlgorithmOFDB::reconstructFromSingleRP(const vector<const CTPPSLocalTrackLite*> &tracks,
